  ...
  <div class="min-h-screen bg-secure-light flex">
    <aside class="w-64 bg-secure-dark text-white hidden md:block">
      ...
      <nav class="mt-6 px-4 space-y-2">
        ...
        <NuxtLink to="/admin/laporan/member" class="block py-2 px-4 rounded bg-secure-gold text-secure-dark font-semibold">Laporan Member</NuxtLink>
    ...
    <main class="flex-1 p-8">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Laporan Member</h1>
      ...
      <div class="bg-white rounded-lg shadow overflow-hidden" id="print-area">
        <div class="print-only hidden mb-4 text-center">
          <h2 class="text-2xl font-bold">Laporan Member SecureParking</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="m in members" :key="m.id">
              <td class="px-6 py-4 whitespace-nowrap">{{ m.plat_nomor }}</td>
              <td class="px-6 py-4 whitespace-nowrap">{{ m.nama }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="m.status === 'lunas/aktif' ? 'text-green-600 bg-green-100' : 'text-red-600 bg-red-100'" class="px-2 py-1 rounded-full text-xs font-bold">{{ m.status }}</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">{{ m.tanggal_mulai }}</td>
              <td class="px-6 py-4 whitespace-nowrap">{{ m.tanggal_berakhir }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const fetchLaporan = async () => {
  try {
    let url = `${config.public.apiBase}/admin/laporan/member`
